...
import static ru.otus.kupipodariday.validator.user.UserValidator.EMPTY_EMAIL_VALIDATION_EXCEPTION;
...
public class SignUpRequest {
    @NotNull(message = EMPTY_USERNAME_VALIDATION_EXCEPTION)
    ...
    @NotNull(message = EMPTY_EMAIL_VALIDATION_EXCEPTION)
    @Length(min = 5, max = 256, message = EMAIL_VALIDATION_EXCEPTION)
    @Email(message = EMAIL_VALIDATION_EXCEPTION)
    private String email;

    @NotNull(message = EMPTY_PASSWORD_VALIDATION_EXCEPTION)
